...
description: |
  ...
  - Comparing models for requirement writing, design, task breakdown, code generation, or review
  - A model version updated — re-calibrate to see what changed
  - Asking "what can this model do?" or "where does it fit in the workflow?"
  - Running structured capability evaluation across SDLC phases with rubric scoring
  - Building or reading a capability map (baseline → best → ceiling per phase)
  - Iterating prompts to push a model's performance ceiling per SDLC task
  - Running the calibration loop: baseline probe → score → weakness analysis → targeted improvement → plateau check
  - Working with the generation/analysis split for small models (<10B params)
  - Choosing prompt strategies: concrete examples, two-turn conversation, bullet guidance, pre-fill, module-at-a-time scoping, role priming
  - Producing calibration artifacts: phase reports, capability maps, best outputs, optimized prompts
  Trigger on: "calibrate this model", "evaluate Gemma for SDLC", "build a capability map", "which model handles design review", "run the calibration loop", "score against the rubric", "what SDLC roles can model X fill?"
